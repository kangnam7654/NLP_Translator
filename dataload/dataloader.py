...
from dataload.dataset import CustomDataset, BaseCustomDataset
...
class BaseLitDataLoader(LightningDataModule):
    def __init__(self, train_df=None, valid_df=None, test_df=None, tokenizer=None, device=None):
        super().__init__()
        self.train_df = train_df
        self.valid_df = valid_df
    ...
    def __create_dataset(self, mode='train'):
        if mode == 'train':
            return BaseCustomDataset(df=self.train_df, tokenizer=self.tokenizer, train_stage=True)
        elif mode == 'valid':
            return BaseCustomDataset(df=self.valid_df, tokenizer=self.tokenizer, train_stage=True)
        elif mode == 'test':
            return BaseCustomDataset(df=self.test_df, tokenizer=self.tokenizer, train_stage=False)
